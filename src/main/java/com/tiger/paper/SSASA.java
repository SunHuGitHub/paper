...
//            }
            ...
            if (i > speciesNum * 1.0 / 2) {
                ...
                do {
                    sparrowIndex = BigDecimal.valueOf(randomNormalDistribution() *
                            Math.exp((((updateMap.get("globalMin").doubleValue()) - temp) / Math.pow(i, 2))))
                ...
                do {
                    sparrowIndex = BigDecimal.valueOf(pdMax.doubleValue() + BigDecimal.valueOf(Math.abs(temp - pdMax.doubleValue())).doubleValue() * BigDecimal.valueOf(A[Math.random() > 0.5 ? 1 : 0] * 1).doubleValue()).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                } while (sparrowIndex <= 0 || sparrowIndex >= 1);
    ...
    private void rankAndFindLocation() {
        ...
        for (int i = 2; i <= speciesNum; i++) {
            ...
            if (i <= PD && fTime(pdMax) < f) {
                pdMax = sparrowIndex;
            }
            if (fTime(globalMin) < f) {
                globalMin = sparrowIndex;
            }
            if (fTime(globalMax) > f) {
