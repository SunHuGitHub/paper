...
public class SSATS {
     ...
     * 禁忌表
     */
    private List<Double> tabooTable;
    ...
    public SSATS(int speciesNum, int iterations, double PDRatio, double SDRatio, double ST) {
        ...
        this.tabooTable = new ArrayList<>((int) Math.sqrt(speciesNum));
    ...
    private void updateSDPoint() {
        ...
        for (int i = 0; i < sdPoints.size(); i++) {
            ...
            if (f > fg) {
                do {
                    sparrowIndex = globalMax + randomNormalDistribution() * Math.abs(globalMax - sparrowIndex);
                } while (sparrowIndex < 0 || sparrowIndex > 1);
            } else if (Math.abs(f - fg) <= 1e-10) {
                do {
                    sparrowIndex = sparrowIndex + Math.random() * (Math.abs(sparrowIndex - globalMin) / (fw - f + 1e-4));
                } while (sparrowIndex < 0 || sparrowIndex > 1);
            }
    ...
    private void updateProduceMap() {
        ...
        updateMap.put("pdMax", packagingAccuracy(pdPointsTemp.get(0)));
    }
                ...
                if (Math.abs(lastf - fpdMax) / fpdMax <= 0.2) {
                    while (n <= searchFields) {
                        ...
                        if (!tabooTable.contains(newPdIdx)) {
                            tabooTable.add(newPdIdx);
                            n++;
                        }
                    }
                    List<Double> tabooTableTemp = new ArrayList<>(tabooTable);
                    tabooTableTemp.sort(new Comparator<Double>() {
                        @Override
                        public int compare(Double o1, Double o2) {
                            return f(o2) - f(o1) > 0 ? 1 : -1;
                        }
